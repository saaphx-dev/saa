...
function saa_basic_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = saa_basic_id_safe('page-'. $path);
    $vars['classes_array'][] = saa_basic_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('saa_basic_ie_enabled')) {
    $saa_basic_ie_enabled_versions = theme_get_setting('saa_basic_ie_enabled_versions');
    if (in_array('ie8', $saa_basic_ie_enabled_versions, TRUE)) {
...
function saa_basic_preprocess_page(&$vars, $hook) {
...
function saa_basic_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from saa_basic_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function saa_basic_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function saa_basic_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('saa_basic_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('saa_basic_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('saa_basic_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('saa_basic_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('saa_basic_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function saa_basic_id_safe($string) {
 ...
 *
 */
function saa_basic_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = saa_basic_id_safe($element['#title']);
 ...
 */
function saa_basic_preprocess_menu_local_task(&$variables) {
 ...
 */
function saa_basic_menu_local_tasks(&$variables) {
  $output = '';
